...
#Region "VariablesGlobales"
        ...
        'Private Shared colores() As Integer = {&H808080, &H9CAEC3, &H73846E, &HB6C9C4, &H9A8273, &H5F7F6A, &HA8A73B, &H8899AA, &HB69BC1, &HAA8822, &HDD8866, &H55C134, &H0}
...
#Region "Parametrización de Graficas WebChartView"
        ...
        Public Sub subGraficarExpReal(ByVal arrayDatosGrafSismoX As Double(), ByVal arrayDatosGrafSismoY As Double(), ByVal intColor As Integer, ByVal strNombreLinea As String, ByRef grfGrafica As XYChart)
            ...
            linLayerGrafSimul.setLineWidth(2)
        End Sub
...
#Region "GenerarGraficasDinamicamente"
        ...
        ''' </summary>
        ''' <param name="bouArrayTargetSpecSismos">Matriz tridimencional para el "Target Sprectrum" y "los Sísmos"</param>
        ''' <remarks></remarks>
        Public Sub subConstruirGraficas(ByVal bouArrayTargetSpecSismos(,,) As Double, ByVal intColumna As Integer)

            Dim intNumSismos As Integer = UBound(bouArrayTargetSpecSismos, 1)
            Dim intNPTS As Integer = UBound(bouArrayTargetSpecSismos, 2)
            Dim arrayDatosGrafPeriodoX(intNPTS) As Double
            Dim arrayDatosGrafAcerlDesplazY(intNPTS) As Double
            XYChart_Grafica_Sismo = New XYChart(intAnchoGraficas, intAltoGraficas, intColorFondo, intColorFondo, 0) 'XYChart para el primer gráfico
            wcvSismo = New WebChartViewer
            'Establece las propiedades de cada WebChartView creado dinamicamente
            establecerPropCtrlGraficos(XYChart_Grafica_Sismo)
            ' ---------------------------------------------------------------------------
            '| AGREGA LA LINEA TARGET: GRÁFICA "ACELERACION Y DESPLAZAMIENTO ESPECTRAL" |
            ' ---------------------------------------------------------------------------
            For j As Integer = 0 To intNPTS
                arrayDatosGrafPeriodoX(j) = bouArrayTargetSpecSismos(0, j, 0)
            Next
            For j As Integer = 0 To intNPTS
                arrayDatosGrafAcerlDesplazY(j) = bouArrayTargetSpecSismos(0, j, intColumna)
            Next
            'A la grafica construida enel paso anterior, se le agrega los datos del sísmo
            subGraficarExpReal(arrayDatosGrafPeriodoX, arrayDatosGrafAcerlDesplazY, UBound(colores), "TARGET", XYChart_Grafica_Sismo)
            ' ------------------------------------------------------------------------------------
            '| AGREGA LAS LINEA LINEAS SISMICA: GRÁFICA "ACELERACION  Y DESPLAZAMIENTO ESPECTRAL" |
            ' ------------------------------------------------------------------------------------
            For i As Integer = 1 To intNumSismos
                For j As Integer = 0 To intNPTS
                    arrayDatosGrafPeriodoX(j) = bouArrayTargetSpecSismos(i, j, 0)
                Next
                For j As Integer = 0 To intNPTS
                    arrayDatosGrafAcerlDesplazY(j) = bouArrayTargetSpecSismos(i, j, intColumna)
                Next
                'A la grafica construida enel paso anterior, se le agrega los datos del sísmo
                subGraficarExpReal(arrayDatosGrafPeriodoX, arrayDatosGrafAcerlDesplazY, i - 1, "Earthquake " & i, XYChart_Grafica_Sismo)
            Next
            'Se construye la imagen
            wcvSismo.Image = XYChart_Grafica_Sismo.makeWebImage(Chart.PNG)
            'Se agrega la imagen "wcvSismo" a la tabla
            subAddWcvSismos(wcvSismo)
        End Sub
